 ...
 * ----------------------------------------------
 * 官方网址: http://http://www.easybigu.com/
 ...
 * 服务发布中的第一步，选择服务分类
 */
var goods_category_quick = new Array();// 快速选择服务分类集合
$(function() {
...
//	}
	...
	// 查询当前用户最近一天是否发布过服务
	$.ajax({
		url :  __URL(ADMINMAIN+"/services/getquickgoods"),
		type : "post",
...
function loadingQuick(categoryID, index) {
	...
	$.ajax({
		url : __URL(ADMINMAIN+"/services/getchildcategory"),
		type : "post",
		async : false,
		data : {
...
//	var quick_id = "";// 所选择的服务分类
//	for (var i = 1; i < count; i++) {
//		var span = $(spanList[i]);
//		var html = span.html();
//		goods_category_name += html;
//		quick_id += span.attr("cid") + ",";// 记录用户所选择的服务类目Id，用与在快速选择服务类目中显示
//	}
...
//	// 判断当前所选择的服务分类与Cookie中的进行查询，是否存在，不存在则添加，
...
function GetProductCategoryData(categoryData) {
	...
	for (var i = 0; i < categoryData.length; i++) {
		...
		var attrId = cate["attr_id"];//关联服务类型ID
		var $rootCategory = "";
