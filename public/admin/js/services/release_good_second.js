 ...
 * ---------------------------------------------- 官方网址:
 * http://http://www.easybigu.com/ 这不是一个自由软件！您只能在不用于商业目的的前提下对程序代码进行修改和使用。
 * 任何企业和个人不允许对程序代码以任何形式任何目的再发布。
 * =========================================================
 * 
 * @author : 小学生王永杰
 * @date : 2016年12月16日 16:17:13
 * @version : v1.0.0.0 服务发布中的第二步，编辑服务信息
 */
$(function() {
	if(parseInt($("#goodsId").val()) > 0){
		...
		if(parseInt($("#goodsType").val()) == 0 ){
			//编辑服务时，有些服务没有选择服务类型
			$("#goodsType").attr("data-flag",1);//标识 0：表示添加服务，1：表示编辑服务，服务分类为0,2：表示编辑服务，服务分类不为0
			getGoodsSpecListNotAttrId();
		}else{
			$("#goodsType").attr("data-flag",2);//标识 0：表示添加服务，1：表示编辑服务，服务分类为0,2：表示编辑服务，服务分类不为0
			/*getGoodsSpecListNotAttrId();
			getGoodsAttributeListByAttrId($("#goodsType").val(), function(){
				$(".js-goods-sku-attribute tr").each(function(){
					var value = $(this).children("td:first").attr("data-value");//服务属性名称
		...
		$("#goodsType").attr("data-flag",0);//标识 0：表示添加服务，1：表示编辑服务，服务分类为0,2：表示编辑服务，服务分类不为0
...
//		if(parseInt($("#goodsType").val())>0){
			OpenSkuDialogServices(ADMINMAIN,parseInt($("#goodsType").val()));
...
//		}
     ...
     * 2017年6月6日 09:46:46 王永杰
     */
    $(".js-xinxi-add").live("click",function(){
        if($(this).attr("data-flag") == undefined){
            $(".js-xinxi-add").html("添加规格值").removeAttr("data-flag style");
...
//					});
    ...
    $("input[name='integralSelect']").change(function() {
        ...
        } else if($("input[name='integralSelect']:checked").val() == 2){
	...
	$("#tbcNameCategory,#tbcExtendNameCategory").live("click",function(){
		...
		OpenCategoryDialogServices(ADMINMAIN,category_id,goodsid,flag);
	});
	...
	$('#img_box').live('click',function(e){
		...
		shopImageFlag = js_img;//所点击的服务图片标识
...
function addGoodsSpecValue(spec,callBack){
	$.ajax({
		url : __URL(ADMINMAIN+"/services/addGoodsSpecValue"),
		type : "post",
...
function getGoodsSpecListByAttrId(attr_id,callBack){
	if(!isNaN(attr_id) && attr_id > 0){
		$.ajax({
			url : __URL(ADMINMAIN+"/services/getGoodsSpecListByAttrId"),
			type : "post",
		...
		//标识 0：表示添加服务，1：表示编辑服务，服务分类为0,2：表示编辑服务，服务分类不为0
...
function ValidateUserInput() {
	...
	if (!IsEmpty("#txtProductTitle")) {
		...
		//$("#txtProductTitle").next("span").show();
		$("#txtProductTitle").nextAll("span:last").text("服务标题不能大于60个字").css("color","red");
	...
	/*if (!IsNum("#txtProductSalePrice") || parseFloat($("#txtProductSalePrice").val()) < 0.01) {
		$("#txtProductSalePrice").nextAll("span:last").text("服务销售价不能为空，且大于0").css("color","red");
...
//		$("#txtProductMarketPrice").nextAll("span:last").text("服务市场价不能为空，且大于0");
...
//		$("#txtProductCostPrice").nextAll("span:last").text("服务成本价不能为空，且大于0");
...
//		}
	...
	if(isflag == 0){
		...
		if (!IsNum("#txtProductSalePrice") || parseFloat($("#txtProductSalePrice").val()) < 0) {
			$("#txtProductSalePrice").nextAll("span:last").text("服务销售价不能为空，且不能为负数").css("color","red");
			$("#txtProductSalePrice").nextAll("span:last").show();
			$("#txtProductSalePrice").focus();
			return false;
		} else {
...
//	}
	// 服务图
//	if (imgflag) {
//		$(".example span").text('需要设置第一张图片为服务主图').show();
...
//			if (id == temp_arr[j]) {
//				$(".example span").text('服务图片存在重复').show();
//				$(".example span").addClass('error');
//				$("body").scrollTop($("#file_upload_img_1").offset().top-300);
//				return false;
//			}
...
//	}
	...
	if (description == "") {
		$("#tareaProductDiscrip").nextAll("span:last").text("服务描述不能为空");
		...
		$("#tareaProductDiscrip").nextAll("span:last").text("服务描述字符数应在5～25000之间");
		$("#tareaProductDiscrip").nextAll("span:last").show();
		$("body").scrollTop($("#discripContainer").offset().top-100);
		return false;
	} else {
	...
	$(".ladder_preference").each(function(){
		...
		if(preference >= min_price){
			showTip("优惠价格不能大于或等于服务最小价格","error");
...
//		return;
		$.ajax({
			url : __URL(ADMINMAIN + "/services/GoodsCreateOrUpdate"),
			type : "post",
			async : false,
			data : { "product" : JSON.stringify(productViewObj) , "is_qrcode" : $qrcode},
			dateType : "json",
			success : function(res) {
				var url = __URL(ADMIN_MAIN + "/services/goodslist");
				...
				if (res != null) {
					if (type == 1) {
						...
						url = __URL(SHOP_MAIN + "/services/goodsinfo?goodsid="+parameter_goodsid);// 跳转到前台
						window.open(url);
					}
					showMessage('success', "服务发布成功",__URL(ADMIN_MAIN +'/services/goodslist'));
				} else {
					showMessage('error', "服务发布失败",url);
					flag = false;
...
function PackageProductInfo() {
	...
	productViewObj.goodsId = $("#goodsId").val();// 服务id 11号目前为死值 0
	productViewObj.title = $("#txtProductTitle").val().replace(/^\s*/g, "").replace(/\s*$/g, "");// 服务标题
	productViewObj.goods_type = $("input[name='goods_type']:checked").val();
	productViewObj.introduction = $("#txtIntroduction").val().replace(/^\s*/g, "").replace(/\s*$/g, "");// 服务简介，促销语
	productViewObj.categoryId = $("#tbcNameCategory").attr("cid");// 服务类目 
	...
	productViewObj.categoryExtendId = category_extend_id;// 服务扩展类目
	// 12号 服务类目；
	...
	productViewObj.libiary_goodsid = $("#libiary_goodsid").val(); // 服务库id
	...
	productViewObj.code = $("#txtProductCodeA").val();// 服务编码
	...
	productViewObj.imageArray = imageVals;// 服务图片分组
	//sku规格图片
    ...
    if(productViewObj.point_exchange_type == 2){
        productViewObj.integration_available_use = $("#integration_money_use").val() == '' ? 0 : $("#integration_money_use").val();
    }
	productViewObj.province_id = $("#provinceSelect").val();// 服务所在地：省
	productViewObj.city_id = $("#citySelect").val();// 服务所在地：市
	...
	productViewObj.virtual_goods_type_id = $("#virtual_goods_type_id").val();//虚拟服务类型id
...
function addExtentCategoryBox(){
	...
	html += '<span class="help-inline" style="vertical-align: top;">已添加的服务扩展分类不能为空</span>';
