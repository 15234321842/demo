...
//
    ...
    //转售最小价
    if (!IsNum("#tominprice") || parseFloat($("#tominprice").val()) < 0.01) {
        $("#tominprice").nextAll("span:last").text("最小转售价不能为空，且大于0");
        $("#tominprice").nextAll("span:last").show();
        $("#tominprice").focus();
        return false;
    } else {
        var price_s = $("#tominprice").val();
        var c_price = parseFloat(price_s);
        $("#tominprice").nextAll("span:last").hide();
    }

    //转售最大价
    if (!IsNum("#tomaxprice") || parseFloat($("#tomaxprice").val()) < parseFloat($("#tominprice").val())) {
        $("#tomaxprice").nextAll("span:last").text("最大转售价不能为空，且大于最小转售价");
        $("#tomaxprice").nextAll("span:last").show();
        $("#tominprice").focus();
        return false;
    } else {
        var price_s = $("#tomaxrice").val();
        var c_price = parseFloat(price_s);
        $("#tomaxprice").nextAll("span:last").hide();
    }
...
function PackageProductInfo() {
	...
	productViewObj.cost_price = $("#txtProductCostPrice").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#tominprice").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// 成本价
	productViewObj.tominprice = $("#tominprice").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#tominprice").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// 成本价
	productViewObj.tomaxprice = $("#tomaxprice").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#tomaxprice").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// 成本价
