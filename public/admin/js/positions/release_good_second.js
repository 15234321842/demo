...
//		if(parseInt($("#goodsType").val())>0){
			OpenSkuDialogPositions(ADMINMAIN,parseInt($("#goodsType").val()));
...
//					});
	...
	$("#tbcNameCategory,#tbcExtendNameCategory").live("click",function(){
		...
		OpenCategoryDialogPositions(ADMINMAIN,category_id,goodsid,flag);
	});
...
function addGoodsSpecValue(spec,callBack){
	$.ajax({
		url : __URL(ADMINMAIN+"/positions/addGoodsSpecValue"),
		type : "post",
...
function getGoodsSpecListByAttrId(attr_id,callBack){
	if(!isNaN(attr_id) && attr_id > 0){
		$.ajax({
			url : __URL(ADMINMAIN+"/positions/getGoodsSpecListByAttrId"),
			type : "post",
...
//		return;
		$.ajax({
			url : __URL(ADMINMAIN + "/positions/GoodsCreateOrUpdate"),
			type : "post",
			async : false,
			data : { "product" : JSON.stringify(productViewObj) , "is_qrcode" : $qrcode},
			dateType : "json",
			success : function(res) {
				var url = __URL(ADMIN_MAIN + "/positions/goodslist");
				...
				if (res != null) {
					if (type == 1) {
						...
						url = __URL(SHOP_MAIN + "/positions/goodsinfo?goodsid="+parameter_goodsid);// 跳转到前台
						window.open(url);
					}
					showMessage('success', "商品发布成功",__URL(ADMIN_MAIN +'/positions/goodslist'));
				} else {
