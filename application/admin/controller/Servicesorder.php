...
use data\service\ServicesOrder\OrderStatus as OrderStatus;
    ...
    public function orderList()
    {
        if (request()->isAjax()) {
            ...
            foreach ($all_status as $k => $v) {
                 ...
                 */
                $child_menu_list[] = array(
                    'url' => "Servicesorder/orderlist?status=" . $v['status_id'],
            ...
            return view($this->style . "ServicesOrder/orderList");
        }
    }
    ...
    public function virtualOrderList()
    {
        if (request()->isAjax()) {
            ...
            foreach ($list['data'] as $k => $v){
                ...
                $list['data'][$k]['info'] = array(json_decode(str_replace('&quot;','"',$v['info']),true));
//                $list['data'][$k]['info'] = str_replace('&quot;','"',str_replace('}','',str_replace('{','',$v['info'])));
//                $list['data'][$k]['info'] = explode(',',$list['data'][$k]['info']);
            }
            return $list;
        } else {
            $status = request()->get('status', '');
            ...
            $child_menu_list = array();
            $child_menu_list[] = array(
                'url' => "Servicesorder/virtualorderlist",
                'menu_name' => '全部',
                "active" => $status == '' ? 1 : 0
            );
            foreach ($all_status as $k => $v) {
                $child_menu_list[] = array(
                    'url' => "Servicesorder/virtualorderlist?status=" . $v['status_id'],
                    'menu_name' => $v['status_name'],
                    "active" => $status == $v['status_id'] ? 1 : 0
                );
            }
            $this->assign('child_menu_list', $child_menu_list);
            return view($this->style . "ServicesOrder/virtualOrderList");
        }
    }
    ...
    public function orderDetail()
    {
        ...
        return view($this->style . "ServicesOrder/orderDetail");
    }
    ...
    public function virtualOrderDetail()
    {
        ...
        $this->assign("order", $detail);
        return view($this->style . "ServicesOrder/virtualOrderDetail");
    }
    ...
    public function orderRefundDetail()
    {
        ...
        return view($this->style . "ServicesOrder/orderRefundDetail");
    }
     ...
     * 买家申请退款
     *
     * @return Ambigous <number, \data\service\shop\Order\Exception, \data\service\shop\Order\Ambigous>
     */
    ...
    public function orderGoodsConfirmRefund()
    {
        ...
        $refund_balance_money = request()->post("refund_balance_money", 0); // 退款现金
    ...
    public function returnSetting()
    {
        ...
        if (request()->isAjax()) {
            ...
            return view($this->style . "Servicesorder/returnSetting");
        }
    }
    ...
    public function printpreviewOfInvoice()
    {
        ...
        return view($this->style . "ServicesOrder/printpreviewOfInvoice");
    }
    ...
    public function printOrder()
    {
        ...
        return view($this->style . "ServicesOrder/printOrder");
    }
    ...
    public function printVirtualOrder()
    {
        ...
        $list = $order_service->getOrderList(1, 0, $condition, '');
        $this->assign("order_list", $list['data']);
        return view($this->style . "ServicesOrder/printVirtualOrder");
    }
...
//        if(!empty($order_info)){
            $xianxia=  new  XianxiaModel();
            $res=  $xianxia->getInfo(['order_id'=>$order_id,'type'=>3],"*");
            $domain_name = \think\Request::instance()->domain();
            if(!empty($res)){
                $res['code']=1;
                $res['path']=$domain_name.'/'.$res['path'];
                return $res;
            }else{
                return $data['code']=-1;
            }
