...
class Order extends BaseController
{
    ...
    public function orderOffLinePay()
    {
        ...
        if ($order_info['payment_type'] == 6) {
            $res = $order_service->orderOffLinePay($order_id, 6, 1);

        } else {
            $res = $order_service->orderOffLinePay($order_id, 10, 1);
            if($res){
                //线下支付修改转账状态
...
/*    $order_info=   $order_model->getInfo(['order_id'=>$order_id],'out_trade_no');*/
    if(!empty($order_id)){
    $xianxia=  new  XianxiaModel();
      $res=  $xianxia->getInfo(['order_id'=>$order_id],"*");
