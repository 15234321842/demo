...
//财务首页
    public function financeindex()
    {
        ...
        $this->assign("goods_list", $goods_rank);
        $this->assign("is_index", true);
        return view($this->style . 'Index/financeindex');
    }
    //kefu首页
    public function servicefinanceindex()
    {
        $debug = config('app_debug') == true ? '开发者模式' : '部署模式';
        $this->assign('debug', $debug);
        $main = \think\Request::instance()->domain();
     ...
     * 2017年7月10日 12:06:01 王永杰
     */
    public function getMemberCommisionWithdrawCount()
    {
        $member = new Member();
        $count = $member->getMemberCommsionWithdrawCount([
            "status" => 0
        ]);
        return $count;
    }
     ...
     * 2017年7月10日 12:06:01 王永杰
     */
    public function getUserxianxiaCount()
    {
        $member = new Member();
        $count = $member->getUserxianxiaCount([
            "status" => 0,
            "type"=>2
        ]);
        return $count;
    ...
    public function getOrderCount()
    {
        ...
        $order_count_array['xianxia'] = $order->getOrderCount([
            'shop_id' => $this->instance_id,
            'order_status' => 6,
            'is_deleted' => 0
        ]); // 线下已转账
...
//专业服务
    public function getServiceCount()
    {
        $order = new OrderService();
        $order_count_array = array();
        $order_count_array['daifukuan'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>2, // 订单类型
            'order_status' => 0
        ]); // 代付款
        $order_count_array['xianxia'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_status' => 6,
            'order_type' =>2, // 订单类型
            'is_deleted' => 0
        ...
        $order_count_array['yishouhuo'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>2, // 订单类型
            'order_status' => 3
        ]); // 已收货
        $order_count_array['yiwancheng'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>2, // 订单类型
            'order_status' => 4
        ]); // 已完成
        $order_count_array['yiguanbi'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>2, // 订单类型
            'order_status' => 5,
            'is_deleted' => 0
        ...
        $order_count_array['all'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>2, // 订单类型
            'is_deleted' => 0
        ]); // 全部订单数量，排除已删除的
    ...
    /**
     * 获取 祭祀订单数量 代付款 待发货 已发货 已收货 已完成 已关闭 退款中 已退款
     */
    public function getServiceOrderCount()
    {
        ...
        $order_count_array['daifahuo'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>1, // 订单类型
            'order_status' => 1
        ]); // 代发货
        $order_count_array['yifahuo'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>1, // 订单类型
            'order_status' => 2
        ]); // 已发货
        ...
        $order_count_array['tuikuanzhong'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>1, // 订单类型
            'order_status' => - 1,
        ]); // 退款中
        $order_count_array['yituikuan'] = $order->getServiceOrderCount([
            'shop_id' => $this->instance_id,
            'order_type' =>1, // 订单类型
            'order_status' => - 2
        ]); // 已退款
