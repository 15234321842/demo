...
use app\appapi\controller\Article;
...
class Config extends BaseController
{
    ...
    public function webConfig()
    {
        ...
        if (request()->isPost()) {
            ...
            $app_version = request()->post("app_version", ""); // app版本
            $path_one = request()->post("path_one", ""); // 路径一
            $path_two = request()->post("path_two", ""); // 路径二
            $retval = $this->website->updateWebSite($title, $logo, $web_desc, $key_words, $web_icp, $web_style_pc, $web_qrcode, $web_url, $web_phone, $web_email, $web_qq, $web_weixin, $web_address, $third_count, $web_popup_title, $web_wechat_share_logo, $web_gov_record, $web_gov_record_url,$app_version,$path_one,$path_two);
     ...
     * 线下支付配置
     */
    public function payOFFLINEConfig()
    {
        $web_config = new WebConfig();
        if (request()->isAjax()) {
            // 线下支付
            $bank_user = str_replace(' ', '', request()->post('bank_user', ''));
            $bank_number = str_replace(' ', '', request()->post('bank_number', ''));
            $bank = str_replace(' ', '', request()->post('bank', ''));
            $is_use = request()->post('is_use', 0);
            // 获取数据
            $retval = $web_config->setOFFLINEpayConfig($this->instance_id, $bank_user, $bank_number, $bank, $is_use);
            return AjaxReturn($retval);
        }
        $data = $web_config->getOFFLINEpayConfig($this->instance_id);
        $this->assign("config", $data);
        ...
        return view($this->style . "Config/payOFFLINEConfig");
    }
    /**
     * 设置微信和支付宝开关状态是否启用
     */
...
//        print_r($mobile_message);die();
    ...
    public function shopSet()
    {
        ...
        if (request()->isAjax()) {
            ...
            $to_order_isopen = request()->post("to_order_isopen", 0); // 是否开启二次转售
            $opentoselltime = request()->post("opentoselltime", 10); // 允许二次转售的时间
            $is_platform_deposit = request()->post('is_platform_deposit',''); //是否开启平台寄存
            $retval = $Config->SetShopConfig($shop_id, $order_auto_delinery, $order_balance_pay, $order_delivery_complete_time, $order_show_buy_record, $order_invoice_tax, $order_invoice_content, $order_delivery_pay, $order_buy_close_time, $buyer_self_lifting, $seller_dispatching, $is_logistics, $shopping_back_points, $is_open_virtual_goods, $order_designated_delivery_time,$to_order_isopen,$opentoselltime,$is_platform_deposit);
            return AjaxReturn($retval);
        } else {
            // 订单收货之后多长时间自动完成
            $shop_id = $this->instance_id;
    ...
    public function notice()
    {
        ...
        $article = new \data\service\Article();
        if (request()->isAjax()) {
            $title = request()->post('title', '');
            $content = request()->post('content', '');
            $result = $article->addNotice($title,$content);
            return AjaxReturn($result);
        } else {
    ...
    public function noticelist()
    {
        ...
        if (request()->isAjax()) {
            ...
            $result = $article->getnoticeList($page_index, $page_size, $condition, 'message_id desc');
            return $result;
        } else {
            return view($this->style . 'Config/noticelist');
        }
    }
    //修改站内通知
    public function updatenotice(){
        ...
        if (request()->isAjax()) {
            ...
            $result = $article->updateNotice($title,$content,$message_id);
            return AjaxReturn($result);
        } else {
            ...
            $info = $message_model->where($condition)->find();
            $info['message_id']=$message_id;
            $this->assign('info', $info);
            return view($this->style . 'Config/updateUsernotice');
