...
class Pay extends BaseController
{
    ...
    public function getPayValue()
    {
        ...
        if($deviceType=='mobile' || $deviceType==''){
            $pay_config['ali_pay_config']['is_use']=0 ;
        }
        ...
        if ($pay_value['pay_status'] == 1) {
            // 订单已经支付
            $this->success("订单已经支付或者订单价格为0.00，无需再次支付!");
        }
        if ($pay_value['type'] == 1) {
            // 订单
    ...
    public function companyPaymentpx(){
        ...
        $out_trade_no1= request()->get('out_trade_no', '');
        $out_trade_no2= request()->post('out_trade_no', '');
        $out_trade_no = empty($out_trade_no1)? $out_trade_no2:$out_trade_no1;
        if(empty($out_trade_no)){
            $this->error($deviceType);
        }
                      ...
                      function(res){
                        //alert(JSON.stringify(res));
                         ...
                         if(res.err_msg == "get_brand_wcpay_request:ok" ) {
                            //alert('支付成功');
                            window.location.href = '/lingshanApp/index.html#/orderfinish/'+{$out_trade_no}+'/';
                         }else if(res.err_msg == "get_brand_wcpay_request:fail"){
                            alert('支付失败');
                            window.location.href = '/lingshanApp/index.html#/userorder/tab-2/';
                            //alert(JSON.stringify(res));
...
EOT;
    ...
    public function payCallpay(){
        $out_trade_no1 = request()->get('out_trade_no','');// 流水号
        $out_trade_no2 = request()->post('out_trade_no', '');
        $out_trade_no = empty($out_trade_no1)? $out_trade_no2:$out_trade_no1;
        if(empty($out_trade_no)){
            $this->error('交易号不能为空!');
...
//            校验返回的订单金额是否与商户侧的订单金额一致。修改订单表中的支付状态。
            file_put_contents('pay_error111.txt',$arr['out_trade_no']);
    ...
    public function aliUrlBack(){
        $out_trade_no1 = request()->get('out_trade_no','');// 流水号
        $out_trade_no2 = request()->post('out_trade_no', '');
        $trade_no1 = request()->post('trade_no', '');
        $trade_no2 = request()->get('trade_no', '');

        $out_trade_no = empty($out_trade_no1)? $out_trade_no2:$out_trade_no1;
        $trade_no = empty($trade_no1)? $trade_no2:$trade_no1;
