                ...
                if ($image_size) {
                    ...
                    switch ($this->file_path) {
                        case UPLOAD_GOODS:
                            ...
                            // 上传到相册管理，生成四张大小不一的图
                            $retval = $this->photoCreate($this->reset_file_path, $this->reset_file_path . $newfile, "." . $file_name_explode[$suffix], $type, $pic_name, $album_id, $width, $height, $pic_tag, $pic_id,  ["domain"], $ok["bucket"], $ok["path"]);
                            if ($retval > 0) {

                                $this->success('上传成功',$ok["path"]);
                            } else {

                                $this->error('上传失败',$ok["path"]);
                            }
                    ...
                    $this->error('请检查您的上传参数配置或上传的文件是否有误');
                }
            } else {
                switch ($this->file_path) {
                    case UPLOAD_VIDEO:
        ...
        switch ($this->file_path) {
            case UPLOAD_GOODS:
                ...
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpeg" && $this->file_type != "image/jpg") || $this->file_size > 3000000) {
                   // $this->return['message'] = '文件上传失败,请检查您上传的文件类型,文件大小不能超过3MB';
                    $this->error('文件上传失败,请检查您上传的文件类型,文件大小不能超过3MB');
                    $flag = false;
                }
                break;
            case UPLOAD_GOODS_SKU:
                ...
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpeg" && $this->file_type != "image/jpg") || $this->file_size > 1000000) {
                  //  $this->return['message'] = '文件上传失败,请检查您上传的文件类型,文件大小不能超过1MB';
                    $this->error('文件上传失败,请检查您上传的文件类型,文件大小不能超过1MB');
                    $flag = false;
                }
                break;
            case UPLOAD_GOODS_BRAND:
            ...
            case UPLOAD_VIDEO:
                ...
                if ($this->file_type != "video/mp4" || $this->file_size > 500000000) {
                    $this->error('文件上传失败,请检查您上传的文件类型,文件大小不能超过500MB');
                    $flag = false;
                }
                break;
            case UPLOAD_COMMENT:
