<?php
	...
	class Model
	{
	    ...
	    //是否强制检查字段匹配
	    protected $forceCheckFields   =   true;
	    ...
	    public function __construct($name='')
	    {
	        ...
	        if(!empty($this->tableName) && empty($this->fields)){
	        	Log::record('请填写模型'.$this->name.'的表字段','DB_NOTIC');
	        	return ;
	        }
	    ...
	    protected function _checkTableInfo() {
	        ...
	        if(empty($this->fields)) {
	            ...
	            if($this->forceCheckFields){
	            	Log::record('请填写你的模型字段:'.$this->name,'DB_NOTIC');
	            } 
	            // 如果数据表字段没有定义则自动获取
	            if(C('DB_FIELDS_CACHE')) {
	                $this->fields = F($this->name);
	    ...
	    public function __call($method,$args) {
	        if(in_array(strtolower($method),array('field','table','where','order','limit','page','having','group','lock','distinct'),true)) {
				...
				Log::write($this->name.'->'.$method.' is not exsits.','MODEL_ERR');
				if(C('DEV_MOD')){
					echo $this->name.'->'.$method.' is not exsits.';
					$e = new \Exception;
	     ...
	     * @return mixed
	     +----------------------------------------------------------
	     */
	    public function add($data='',$options=array(),$replace='INSERT') {
	        if(empty($data)) {
	            // 没有传递数据，获取当前数据对象的值
	            if(!empty($this->data)) {
	                $data    =   $this->data;
	    ...
	    public function rAdd($data =[],$options=[]){
	    ...
	    public function regex($value,$rule) {
	        $validate = array(
	            ...
	            'double' => '/^[-\+]?\d+(\.\d+)?$/',
	            'english' => '/^[A-Za-z]+$/',
	            'mobile'=> '/^[1]([0-9])[0-9]{9}$/',  //不严谨版
	            // 'mobile'=> '/^[1](([3][0-9])|([4][5-9])|([5][0-3,5-9])|([6][5,6])|([7][0-8])|([8][0-9])|([9][1,8,9]))[0-9]{8}$/',  //2019严谨版
	            'tel'=>'/^([0-9]{3,4}-)?[0-9]{7,8}$/',
	        );
	        if($rule == 'string'){
	    ...
	    public function validate($data,$required=[],$allowEmpty=[]){ 
	    	//TODO CSRF验证
	    	// 表单令牌验证 还能拿来做重复提交的php检查
	        if(C('TOKEN_ON')) {
	        	$token_name = C('TOKEN_NAME');
	        	$key = $token_name.$data[$token_name];

	        	if( cache($key) != 1 ){
	        		$this->error = array('require'=>[],'format'=>[],'token'=>1,'post'=>$data[$token_name]);
	        		Log::record('check token->post:'.$data[$token_name].' not exisit at:'.$this->getModelName(),'token_error');	
	            	return false;	
	        	}
	        	C('TOKEN_ON',false);
	        ...
	        //去掉不在表中的字段
	        foreach($data as $k=>$v){
	        	if(!in_array($k,$this->fields)) unset($data[$k]);
	        }
	    	return $data;
	    }
	    ...
	    public function data($data){
	        if(is_object($data)){
	        	...
	        	Log::write('数据类型不符合，必需为数组形式','DB_ERR');
	        }
	        return $this;
	    }
	     ...
	     * @return mixed
	     +----------------------------------------------------------
	     */
	     public function findPage($pageopt=0,$count=false,$options=array()) {
			// 分析表达式
	        $options =  $this->_parseOptions($options);
			...
			if($count===false){
	            ...
	            $result = $this->db->selectCount($options);
			...
			if($count > 0) {
				...
				if( is_numeric($pageopt) ) {
					...
					$pagesize	=	intval(C('DEFAULT_LIST_NUMBERS'));
				}
	            ...
	            if($query) {
	                if(substr($query, 0, 12) == 'CREATE TABLE') {
	                    ...
	                    $_temp_query = preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $query).
	                        (mysqli_get_server_info(null) > "4.1" ? " ENGINE=$type DEFAULT CHARSET=$db_charset" : " TYPE=$type");
